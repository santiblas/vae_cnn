...
from models.convolutional_vae2_qc import ConvolutionalVAE
from analysis_qc import summarize_distribution_stages, evaluate_scanner_leakage
...
def scanner_leakage_simple(
    ...
    # calcular cuántos samples mínimos hay por sitio
    # (factorizamos etiquetas para usar bincount)
    y_site_factorized, _ = pd.factorize(y_site)
    class_counts = np.bincount(y_site_factorized)
    min_class_count = int(class_counts.min())

    # número de folds máximo permitido por estratificación
    max_folds_allowed = max(2, min(5, N, min_class_count))

    cv_inner = StratifiedKFold(
        n_splits=max_folds_allowed,
        shuffle=True,
...
def train_and_evaluate_pipeline(global_tensor_all_channels: np.ndarray, 
    ...
    for fold_idx, (train_dev_clf_idx_in_cn_ad_df, test_clf_idx_in_cn_ad_df) in enumerate(outer_cv_clf.split(X_classifier_subject_indices_in_cn_ad_df, stratify_key_for_clf_cv)):
        ...
        history_data = {
            ...
            # val_loss  = recon + beta_actual * KLD  (solo para logging)
            # val_loss_modelsel = recon + beta_max  * KLD (constante)  ⬅️ usar esta para early stopping / checkpoint
        ...
        # =========================
        # QC 1: Distribuciones raw / norm / recon (por canal)
        # =========================
        if args.qc_analyze_distributions:
            try:
                ...
                # Reconstrucciones con el modelo final
                vae_fold_k.eval()
                with torch.no_grad():
                    sample_input = torch.from_numpy(norm_subset).float().to(device)
                    recon_out, _, _, _ = vae_fold_k(sample_input)
                    recon_subset = recon_out.detach().cpu().numpy()
        ...
        with torch.no_grad():
            ...
            if args.qc_check_scanner_leakage:
                try:
                    ...
                    _leak_df = evaluate_scanner_leakage(
                        ...
                        latent_mu_subjects=mu_train_dev.cpu().numpy(),
                        out_dir=fold_output_dir,
            ...
            ############################################################
            ### NUEVO BLOQUE QC LATENTE POR FOLD (para notebook Fig2) ###
            ############################################################

            # 1) Silhouette AD vs CN en el espacio latente del TEST externo
            #    OJO: usamos SIEMPRE mu_test_final (aunque luego combines 'z' o agregues metadatos
            #    al clasificador). Esto hace que la métrica sea comparable entre folds/papers.
            # Para QC interpretabilidad reportamos SIEMPRE mu, aunque el clasificador haya usado z o z+metadata
            latent_for_qc = mu_test_final.cpu().numpy()


            silhouette_latent = compute_latent_silhouette(
                latent_feats = latent_for_qc,
                labels_binary = y_test_final
            )

            # 2) Scanner leakage estimado en el TEST externo
            #    - acc_site_raw: qué tan bien se predice sitio desde conectoma norm.
            #    - acc_site_latent: idem pero desde espacio latente.
            #    - n_sites: cuántos sitios únicos hay.
            qc_leak = scanner_leakage_simple(
                metadata_df_full      = metadata_df_full,
                subject_global_indices= global_indices_clf_test_this_fold,
                X_norm_subjects       = X_test_final_tensor_norm,
                X_latent_subjects     = mu_test_final.cpu().numpy(),
                random_state          = args.seed + fold_idx + 202
            )

            acc_site_raw    = qc_leak["acc_site_raw"]
            acc_site_latent = qc_leak["acc_site_latent"]
            n_sites         = qc_leak["n_sites"]
            ...
            ############################################################
            ### FIN BLOQUE QC LATENTE POR FOLD                      ###
            ############################################################
        ...
        for current_classifier_type in args.classifier_types:
            ...
            if X_latent_test_final.shape[0] > 0:
                ...
                # =================================================================
                # === INICIO: CÓDIGO AÑADIDO PARA GUARDAR PREDICCIONES DE TEST ===
                # =================================================================
                # (Esto también guarda el CSV que usa la Celda A del notebook)
                ...
                logger.info(f"    Predicciones de test (CSV) guardadas en: {pred_save_path}")
                # =================================================================
                # === FIN: CÓDIGO AÑADIDO ===
                # =================================================================
                ...
                # =================================================================
                # === INICIO: CÓDIGO AÑADIDO PARA GUARDAR PREDICCIONES (JOBLIB) ===
                # =================================================================
                # (Esta es la línea que faltaba para popular all_folds_clf_predictions)
